...
#define OLED_SCL(X)    BIT_ADDR(GPIOB_ODR_Addr,6)=X
#define OLED_SDA(X)    BIT_ADDR(GPIOB_ODR_Addr,7)=X
#define OLED_RST(X)    BIT_ADDR(GPIOB_ODR_Addr,8)=X
#define OLED_DC(X)     BIT_ADDR(GPIOB_ODR_Addr,9)=X
#define OLED_CS(X)     0//BIT_ADDR(GPIOB_ODR_Addr,12)=X
...
//    gpio_init (OLED_CS_PIN , GPO,1);
  ...
  RCC_APB2PeriphClockCmd( RCC_APB2Periph_GPIOB , ENABLE); 	//使能gpioc外设时钟					 

    
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;//对一个引脚的初始化过程
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;    
  GPIO_Init(GPIOB, &GPIO_InitStructure);
    
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7;//对一个引脚的初始化过程
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;    
  GPIO_Init(GPIOB, &GPIO_InitStructure);  
    
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;//对一个引脚的初始化过程
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;    
  GPIO_Init(GPIOB, &GPIO_InitStructure);
  
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;//对一个引脚的初始化过程
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;    
  GPIO_Init(GPIOB, &GPIO_InitStructure);
...
void OLED_HEXACSIIFF(u16 hex,u8* Print)
{
	...
	while(TEMP)
	{
	  	...
	  	hexcheck  =  hex%16 ;
	  	hex   /=16 ;
        if(hexcheck>=0&&hexcheck<=9)
	  	   Print[TEMP ]  = hexcheck + 0x30 ;
        else if(hexcheck >=10&&hex<=15)
            Print[TEMP ]  = hexcheck + 0x57 ;
	}
